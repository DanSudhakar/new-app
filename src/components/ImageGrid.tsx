...
export const ImageGrid: React.FC = () => {
  ...
  const visibleImages = images.slice(startIdx, startIdx + 3);

  const handlePrev = () => {
    setStartIdx((prev) => (prev === 0 ? images.length - 3 : prev - 1));
  };

  const handleNext = () => {
    setStartIdx((prev) => (prev + 3 >= images.length ? 0 : prev + 1));
  };

  return (
    <section className="mt-0 w-full max-md:max-w-full">
      <div style={{height:'100%', width: '95.5%',margin:'0 auto'}} className="relative w-full h-[480px] min-h-[480px] bg-black flex items-center justify-center">
        {/* Left Arrow */}
        <button
          className="absolute left-4 top-1/2 -translate-y-1/2 z-10 bg-white/80 hover:bg-white rounded-full p-2 shadow"
          aria-label="Previous"
          onClick={handlePrev}
        >
        ...
        // Center card is idx === 1, left is 0, right is 2
        let scale = 1;
        let rotateY = 0;
        let zIndex = 1;
        let boxShadow = "0 4px 24px rgba(0,0,0,0.2)";
        if (idx === 1) {
          // Center card: smaller, pushed back
          scale = 0.8;
          rotateY = 0;
          zIndex = 0;
          boxShadow = "0 2px 12px rgba(0,0,0,0.15)";
        } else if (idx === 0) {
          // Left card: bigger, rotate left
          scale = 1.1;
          rotateY = 18;
          zIndex = 2;
          boxShadow = "0 8px 32px rgba(0,0,0,0.25)";
        } else if (idx === 2) {
          // Right card: bigger, rotate right
          scale = 1.1;
          rotateY = -18;
          zIndex = 2;
          boxShadow = "0 8px 32px rgba(0,0,0,0.25)";
        ...
        return (
          <div
            key={startIdx + idx}
            className="relative flex-1 h-full transition-transform duration-500"
            style={{
          minWidth: 0,
        ...
        {/* Right Arrow */}
        <button
          className="absolute right-4 top-1/2 -translate-y-1/2 z-10 bg-white/80 hover:bg-white rounded-full p-2 shadow"
          aria-label="Next"
          onClick={handleNext}
        >
          <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-10">
          {images.map((_, idx) => {
            // The "active" dot is the first visible image
            const isActive = idx === startIdx;
            return (
              <button
          key={idx}
          className={`w-3 h-3 rounded-full transition-all duration-300 ${isActive ? "bg-white" : "bg-white/40"}`}
        ...
        {/* <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 pointer-events-none select-none">
          <span className="text-3xl font-bold text-white drop-shadow-lg">Gen AI Tools</span>
        </div> */}
      </div>
    </section>
  );
